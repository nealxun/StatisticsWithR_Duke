...
sfo_feb_flights %>%
        summarise(mean_ad = mean(arr_delay),
                  ...
                  sd_ad = sd(arr_delay))
# Answer: No flight is delayed more than 2 hours is false.
...
sfo_feb_flights %>%
        ...
        summarise(iqr_ad = IQR(arr_delay),
                  n = n())
# answer: Delta and United Airlines have the highest IQR of arrival delays
...
# answer: July has the highest average departure delay from an NYV airport
...
# answer: December has the highest median departure delay from an NYC airport
```
...
df <- nycflights %>% filter(dep_delay <= quantile(nycflights$dep_delay, probs = 0.95))
ggplot(df, aes(x = factor(month), y = dep_delay)) +
  geom_boxplot()
```
...
df <- nycflights %>%
        mutate(avg_speed_hour = distance / (air_time/60)) %>%
        select(avg_speed_hour, tailnum) %>%
        #group_by(tailnum) %>%
        #summarise(avg_speed_hour = mean(avg_speed_hour)) %>%
        arrange(desc(avg_speed_hour))
df
# answer: N666DN has the fastest average speed
```
...
df <- nycflights %>%
        mutate(avg_speed_hour = distance / (air_time/60))
ggplot(data = df, aes(x = distance, y = avg_speed_hour)) +
        geom_point()
# answer: there is an overall positive assoication between distance and average speed
