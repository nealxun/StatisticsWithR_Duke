...
</ol> answer: The left skewness of the data suggests that the students are less likely to rate the professors highly.
...
```{r two-vars-eda}
# score and gender
ggplot(data = evals, aes(x = gender, y = score, fill = gender)) +
        geom_boxplot()
...
# score and rank
ggplot(data = evals, aes(x = rank, y = score, fill = rank)) +
        geom_boxplot()
...
```{r new-mlr-model}
m_bty_rank <- lm(formula = score ~ bty_avg + rank, data = evals)
summary(m_bty_rank)
