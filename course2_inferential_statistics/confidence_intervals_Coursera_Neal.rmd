...
```{r set-seed}
set.seed(2019)                 # make sure to change the seed
```
...
ggplot(samp) +
        geom_histogram(aes(x = area), binwidth = 200)
...
```{r ci}
df <- samp %>%
  summarise(mean = mean(area), sd = sd(area), se = sd(area)/sqrt(n),
