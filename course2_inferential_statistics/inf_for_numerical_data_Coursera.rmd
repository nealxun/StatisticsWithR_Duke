...
##### answer: Both distributions are extremely right skewed.
...
##### answer: <li> $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$ </li>
...
##### answer: We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers.
...
```
The 90% CI widths is narrower than 99% one.
...
```
Since p-value is more than 0.05, we fail to reject the null hypothesis, thus, there is no difference of weight gained between younger and mature mothers.

7. Now, a non-inference task: Determine the age cutoff for younger and mature 
mothers. Use a method of your choice, and explain how your method works.
```{r cutoff-mature}
cutoff_age1 <- max(nc$mage[nc$mature == "younger mom"])
cutoff_age2 <- min(nc$mage[nc$mature == "mature mom"])
cutoff_age <- mean(cutoff_age1, cutoff_age2)

```
Thus, the cutoff age is around `r cutoff_age`.
...
Whether the birth status (premature vs. full-term) yield different baby weight?
```{r pick-your-own}
inference(y = weight, x = premie, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")

# married people tends to go to hospital less
inference(y = visits, x = marital, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")

```
As expected, full-term baby usually is associated with higher weight.
